...
namespace Common
{
    ...
    public static class DTools
    {
        ...
        #region Envío de mails

        /// <summary>
        /// Envía un correo electrónico utilizando el protocolo SMTP con seguridad SSL.
        /// Soporta varios destinatarios, copias y copias ocultas separando los mails con ";".
        /// </summary>
        /// <param name="destinatario">Dirección de mail del destinatario</param>
        /// <param name="remitente">Dirección de mail del remitente</param>
        /// <param name="copia">Dirección de mail del destinatacio con copia</param>
        /// <param name="copiaOculta">Dirección de mail del destinatacio con copia oculta</param>
        /// <param name="titulo">Título del mensaje</param>
        /// <param name="contenido">Contenido del mensaje (soporta HTML)</param>
        /// <param name="usuario">Nombre de usuario de la cuenta que se utiliza para enviar el mail</param>
        /// <param name="contraseña">Contraseña de la cuenta que se utiliza para enviar el mail</param>
        /// <param name="servidor">Dirección del servidor SMTP</param>
        /// <param name="puerto">Puerto SMTP (normalmente el 465)</param>
        /// <param name="ignorarCertificadoInvalido">True ignora la comprobación de validez del certificado del servidor (recomendado)</param>
        /// <returns>True si se ha enviado correctamente o False si error</returns>
        public static bool EnviarCorreo(
            ...
            string servidor,
            int puerto,
            bool ignorarCertificadoInvalido)
        {
            try
            {
                // Creamos el cliente SMTP y lo configuramos
                SmtpClient client = new SmtpClient();
                client.DeliveryMethod = SmtpDeliveryMethod.Network;
                client.Host = servidor;
                client.Port = puerto;
                client.Credentials = new System.Net.NetworkCredential(usuario, contraseña);
                client.EnableSsl = true;

                // Si ignoramos la validación de certificado...
#if !NOVERIFICARCERTIFICADO
                if (ignorarCertificadoInvalido)
                {
                    // Capturamos el CallBack y devolvemos true siempre
                ...
                {
                    var direcciones = destinatario.Split(';');
                ...
                {
                    var direcciones = copia.Split(';');
                    foreach (var dir in direcciones)
                    {
                        mail.CC.Add(dir);
                    }
                }
                // Añadimos las copias ocultas
                if (!string.IsNullOrEmpty(copiaOculta))
                {
                    var direcciones = copiaOculta.Split(';');
                    foreach (var dir in direcciones)
                    {
                        mail.Bcc.Add(dir);
                    }
                ...
                // Enviamos, si no puede enviar salta una excepción
                client.Send(mail);
            ...
            {
                // No ha podido enviar, los detalles en la excepción
                return false;
            }
        }
